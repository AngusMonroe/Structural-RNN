...
def sample_gaussian_2d(mux, muy, sx, sy, corr, nodesPresent):
    ...
    for node in range(numNodes):
        if node not in nodesPresent:
            continue
        mean = (o_mux[node].cpu().numpy(), o_muy[node].cpu().numpy())
        cov = [[o_sx[node].cpu().numpy()*o_sx[node].cpu().numpy(), o_corr[node].cpu().numpy()*o_sx[node].cpu().numpy()*o_sy[node].cpu().numpy()],
               [o_corr[node].cpu().numpy()*o_sx[node].cpu().numpy()*o_sy[node].cpu().numpy(), o_sy[node].cpu().numpy()*o_sy[node].cpu().numpy()]]
